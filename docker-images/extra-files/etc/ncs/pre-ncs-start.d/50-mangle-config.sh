...
DEVEL_LOG_LEVEL=${DEVEL_LOG_LEVEL:-info}
XPATH_TRACE=${XPATH_TRACE:-false}
...
echo "SSH_HOST_KEY_TYPE: ${SSH_HOST_KEY_TYPE}"
echo "DEVEL_LOG_LEVEL: ${DEVEL_LOG_LEVEL}"
echo "XPATH_TRACE: ${XPATH_TRACE}"
...
# Enable or disable the XPath trace log
xmlstarlet edit --inplace -N x=http://tail-f.com/yang/tailf-ncs-config \
            --update '/x:ncs-config/x:logs/x:xpath-trace-log/x:enabled' --value "${XPATH_TRACE}" \
            $CONF_FILE

# Change level for devel.log (default is "info", but for development "trace" is better)
xmlstarlet edit --inplace -N x=http://tail-f.com/yang/tailf-ncs-config \
           --update '/x:ncs-config/x:logs/x:developer-log-level' --value "${DEVEL_LOG_LEVEL}" \
           $CONF_FILE
