* NSO in Docker: NSO system standard form skeleton
  This is the README file for the NSO in Docker (NID) NSO system standard form skeleton. If you see this file (=README.nid-system.org=) in a repository, it means the repository follows the standard form.
...
** Docker tags and prefixes
   ...
   Use the =tag-release= target to set the release tags on the image, e.g. go from =5.3-kll= to =5.3=. The CI configuration automatically does this for CI jobs run on the branch (like =main= or =master=). You might have to do it locally in case you wish to retag images so they can be tested with other repositories.
