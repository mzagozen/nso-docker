...
*NOTE*: There are official Cisco NSO container images offered for download from https://software.cisco.com, which is entirely different from NSO in Docker. See the [[*Comparison of NSO in Docker and the official Cisco NSO container images]].
...
* NSO in Docker for development and production
  ...
  In addition there are some pre-canned git project repository skeletons that help you to quickly establish a project structure and unlock multiple advanced features, like rebuilding packages and reloading in a running NSO container, automatic CI jobs etc. See the [[./skeletons/][NID skeletons]] for how to get started developing in the NID ecosystem.
...
**** Alternative for providing NSO install files into any CI runner
...
**** Alternative for providing NSO install files into GitLab self-hosted CI runner
...
*** Idiomatic container handling of ncs.conf
    ...
    | =DEVEL_LOG_LEVEL=    | enum    | info     | Configure the devel.log level to 'info' or 'trace'                                                    |
...
The NSO in Docker skeletons offer a common way of working for good developer experience (DX). Let's say you have a NSO service package called ~my-service-pkg~. Build it NID-style with:
#+BEGIN_SRC sh
